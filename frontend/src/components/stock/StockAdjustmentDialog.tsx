...
const ADJUSTMENT_TYPES: {
...
}[] = [
  {
    ...
    hoverClass: "text-muted-foreground hover:text-emerald-600 hover:bg-emerald-500/10",
  },
  {
    id: "out",
    label: "Reduce",
    ...
    hoverClass: "text-muted-foreground hover:text-rose-600 hover:bg-rose-500/10",
  },
  {
    id: "adjustment",
    label: "Opname",
    ...
    <>
      ...
      <form onSubmit={handleSubmit} className="space-y-3.5 mt-1">
        <div className="flex items-center justify-between p-2 rounded-xl bg-muted/40 border border-border/70 text-xs select-none">
          {[
            { stepNum: 1 as const, label: "Quantity & Calc", active: step === 1, valid: true },
            { stepNum: 2 as const, label: "Reason & Note", active: step === 2, valid: isStep1Valid },
          ].map((s, idx) => (
            <React.Fragment key={s.stepNum}>
              {idx > 0 && <div className="flex-1 mx-3 h-0.5 bg-border/80 rounded-full" />}
              <button
                type="button"
                disabled={!s.valid}
                onClick={() => s.valid && setStep(s.stepNum)}
                className={cn(
        ...
        {step === 1 ? (
          <div className="space-y-3 animate-in fade-in-50 duration-200">
              ...
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="text-[11px] text-muted-foreground font-medium">
                  ...
                  <strong className="text-foreground font-bold font-mono">
                    {currentQty.toLocaleString("id-ID")} {currentStock?.unit || "units"}
                  </strong>
                </span>
              </div>
            ...
            <div className="space-y-1">
              <Label className="text-xs font-bold text-foreground">Adjustment Type</Label>
            ...
            <div className="space-y-1">
              ...
              <div className="flex items-center gap-1.5">
                <div className="relative flex-1">
                  <Input
                    ...
                    onChange={(e) => {
                      ...
                      if (adjustmentType === "out" && !isNaN(parsed) && parsed > currentQty) {
                        setQuantity(String(currentQty));
                      } else {
                ...
                <div className="flex items-center gap-1 shrink-0">
                  {[5, 10, 20].map((stepVal) => {
                    const isStepDisabled = adjustmentType === "out" && stepVal > currentQty;
                    return (
                      <Button
                        key={stepVal}
                        type="button"
                        ...
                        className={cn(
                          ...
                          isStepDisabled && "opacity-35 cursor-not-allowed hover:border-border",
                        )}
                      >
          ...
          <div className="space-y-3.5 animate-in fade-in-50 duration-200">
            <div className="p-3 rounded-2xl bg-muted/40 border border-border/70 space-y-2.5">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-1.5 sm:gap-2 text-xs pt-2 border-t border-border/40">
                ...
                <div className="flex items-center justify-between sm:justify-end gap-1.5 font-mono">
                  <span className="text-muted-foreground text-[11px]">Final Balance:</span>
            ...
            <div className="space-y-1.5">
              <Label htmlFor="stock-note" className="text-xs font-bold text-foreground">
              ...
              <div className="space-y-1 pt-1">
                ...
                <div className="flex flex-wrap items-center gap-1.5">
                  {PRESET_REASONS.filter((p) => p.type === adjustmentType).map((preset, idx) => {
